...
local function widget()
	...
	local update_uptime = function()
		awful.spawn.easy_async_with_shell("uptime | awk '{ print $3 }' | sed 's/:/ hours /g' | sed 's/,/ minutes/g'", function(stdout)
			...
			nil,
			{
				widgets.button.text.normal({
					...
					on_release = function()
						...
						awesome.emit_signal("central_panel::toggle", mouse.screen)
					end,
				}),
				widgets.button.text.normal({
					forced_width = dpi(50),
					...
					size = 17,
					on_release = function()
						awful.spawn.with_shell("sh ~/.config/rofi/powermenu.sh")
						awesome.emit_signal("central_panel::toggle", mouse.screen)
					end,
				}),
				spacing = dpi(10),
