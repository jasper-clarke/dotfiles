...
--- PLayerctl
--- ~~~~~~~~~
playerctl_daemon:connect_signal("metadata", function(_, title, artist, album_path, album, __, ___, ____, player_name)
	if player_name == "mpd" then
		...
		artwork:set_image(gears.surface.load_uncached(album_path))
			...
			{
				{
					...
					},
					{
						...
						widgets.playerctl.loop_track(beautiful.white, beautiful.music_bg_accent),
						widgets.playerctl.loop_playlist(beautiful.white, beautiful.music_bg_accent),
						spacing = dpi(10),
						layout = wibox.layout.fixed.horizontal,
					},
					{
