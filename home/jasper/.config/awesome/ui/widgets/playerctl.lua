...
function playerctl.loop_track(icon_color, bg_color)
	local playerctl_daemon = general_playerctl_daemon

	local track_widget = tbutton.state({
		normal_shape = gshape.rounded_rect,
		font = beautiful.icon_font .. "Round ",
		size = 12,
		text_normal_bg = icon_color,
		...
		text = "󰑘 ",
		on_release = function()
			playerctl_daemon:cycle_loop_status_track()
		end,
	})

	playerctl_daemon:connect_signal("loop_status_track", function(self, loop_status_track, player)
		if loop_status_track == "none" then
			track_widget:turn_off()
		else
			track_widget:turn_on()
		end
	end)
...
function playerctl.loop_playlist(icon_color, bg_color)
	local playerctl_daemon = general_playerctl_daemon

	local playlist_widget = tbutton.state({
		normal_shape = gshape.rounded_rect,
		font = beautiful.icon_font .. "Round ",
		size = 12,
		text_normal_bg = icon_color,
		...
		on_release = function(self)
			playerctl_daemon:cycle_loop_status_playlist()
		end,
	})

	playerctl_daemon:connect_signal("loop_status_playlist", function(self, loop_status_playlist, player)
		if loop_status_playlist == "none" then
			playlist_widget:turn_off()
		else
			playlist_widget:turn_on()
		end
	end)
